...
sap.ui.controller("cross.fnd.fiori.inbox.CA_FIORI_INBOXExtension.view.MultiSelectSummaryCustom", {
	...
	//    _CUSTOM_ATTRIBUTE_OBJECT: "CustomObjectAttribute",
	    onInit: function () {
	        ...
	        sap.ca.scfld.md.app.Application.getImpl().getComponent().getEventBus().subscribe("cross.fnd.fiori.inbox", "multiselect", this.onMultiSelectEvent, this);
	...
	//        for (var i = 0; i < this.aWorkItems.length; i++) {
	//            if (this.aWorkItems[i].SAP__Origin == w.SAP__Origin && this.aWorkItems[i].InstanceID == w.InstanceID)
	...
	//                this.oCustomerNumberCol = new sap.m.Column({ header: new sap.m.Label({ text: w.CustomNumberLabel }) });
	...
	//                this.oCustomerNumberUnitCol = new sap.m.Column({ header: new sap.m.Label({ text: w.CustomNumberUnitLabel }) });
	...
	//                this.oCustomObjectAttributeCol = new sap.m.Column({ header: new sap.m.Label({ text: w.CustomObjectAttributeLabel }) });
